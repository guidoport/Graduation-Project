...
dataset = np.load('training_FPFH_global.npy')
...
model.add(Dense(X.shape[1]*2, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
...
# fit
history = model.fit(X, Y, validation_split=.33, epochs=50, batch_size=10000, verbose=2, class_weight=class_weight)
