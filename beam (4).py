...
# ds = pd.read_csv('C:/Users/s143873/Documents/Afstuderen/curvtest.csv', delimiter=',')
# ts = pd.read_csv('C:/Users/s143873/Documents/Afstuderen/curvtest.csv', delimiter=',')
#
# print(ds)
#
...
#
# # ds = np.load('C:/Users/s143873/Desktop/Segmented point cloud/11+12/room11+12_merged_test.npy')
#
# print(ds)

# folder_name = glob.glob('*')
# print(folder_name)
...
# object_class = 'window'
object_class = os.path.basename(__file__)[:-3]

dataset = np.load('{}/{}.npy'.format(base_dir, object_class))
testset = dataset
ds = pd.DataFrame(dataset, columns=['x', 'y', 'z'])
size = len(dataset)
...
curvs = np.array([curv_estim(i) for i in range(len(dataset))], dtype=object)
...
orientation_l = np.array(orientation_l, dtype=object)
...
# final_output = pd.concat([, norm_out, curvs_out, orient_out], axis=1)
# print(final_output)
# quit()
# final_output.to_csv('C:/Users/s143873/Documents/Afstuderen/curvtest_with_normals.csv')

# print(dataset.shape)
# print(result_norm.shape)
# print(curvs.shape)
# print(orientation_l.shape)
class_dict = {"beam":0, "ceiling":1, "chair":2, "column":3, "door":4, "floor":5, "occlusion":6, "table":7, "wall":8, "window":9}

classif = np.array(len(dataset) * [class_dict[object_class]], dtype=object)

final_output = np.concatenate((dataset, result_norm, curvs[:,None], orientation_l[:,None], classif[:,None]), axis=1)
print(final_output)

np.save('{}/{}_features.npy'.format(base_dir, object_class), final_output)
